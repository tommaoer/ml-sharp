...
`predict-finetune` tolerates legacy fine-tune checkpoints and ignores `delta_decoder.*` keys if present.
...
For harder overfit cases (single-scene, many steps), enable `--train-prediction-head` to optimize `prediction_head` for sharper mask-region details.
...
Video decoding uses a reused reader per scene to avoid frequent ffmpeg process churn.
To avoid exhausting file descriptors on very large multi-scene runs, the dataset limits concurrent open video readers per worker and evicts old ones automatically.

The command keeps the network input at `1536x1536`. Fine-tuning updates the predictor `feature_model` together with a mask-guided Gaussian refiner, while the other predictor modules stay frozen. The target-view supervision now detects regions that are visible in the target view but invisible from the source view, predicts additive Gaussian deltas for those gated Gaussians, and re-renders the refined target view. Each visualization step saves 8 images: `source.train`, `target.train`, `source_render.train`, `target_render.train`, `loss_region_mask`, `invisible_mask`, `invisible_target_render`, and `gaussian_update_gate`. A `step_000000.*` visualization is still saved at `epoch=0, step=0` before that iteration's optimizer update. By default, fine-tuning uses `--low-pass-filter-eps 0.0` so the debug renders match the standard SHARP render path instead of adding extra smoothing. Fine-tuning currently requires CUDA because the training loop uses differentiable `gsplat` rendering.
